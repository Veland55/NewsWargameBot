...
отредактировать, опубликовать, отклонить или отложить на «Сегодня»/
«Завтра» и конкретное время — удобнее в веб-панели, раздел «Публикация»).
Отложенная новость публикуется сама, без напоминаний. Бот пришлёт
сообщение, как только появится новая новость на согласование. Там же,
в «Публикация» — рассылка произвольного текста сразу во все каналы.
...
async def cmd_checknow(message: Message, st: Storage, publisher: Publisher) -> None:
    ...
    if stats.get("queued") and not hint:
        hint = "\n\n🖐 Новости ждут согласования — /queue или веб-панель, раздел «Публикация»."
...
async def cmd_manual(message: Message, command: CommandObject, st: Storage,
    ...
    if arg in ("on", "вкл", "1"):
        ...
        await _reply(
            ...
            "«Публикация»), кроме отложенных на конкретное время — те "
            "публикуются по расписанию сами, даже на паузе\n"
            ...
            "· в режиме отладки (<code>/debug</code>) публикация вручную "
            "(«Опубликовать всё же», «Повторить сейчас») отключена целиком, "
            "чтобы не уйти в канал мимо согласования по ошибке\n\n"
    ...
    if arg in ("off", "выкл", "0"):
        ...
        extra = (f"\n\n⚠️ В очереди осталось {pending} карточек — сами они не уйдут "
                f"(кроме уже поставленных на конкретное время — те опубликуются по "
                f"расписанию независимо от этой настройки), разберите остальное "
                f"вручную (<code>/queue</code>)." if pending else "")
