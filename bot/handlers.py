...
«Завтра» и конкретное время — удобнее в веб-панели, раздел «Согласование»).
Отложенная новость публикуется сама, без напоминаний. Бот пришлёт
сообщение, как только появится новая новость на согласование.
...
async def cmd_manual(message: Message, command: CommandObject, st: Storage,
    ...
    if arg in ("on", "вкл", "1"):
        ...
        await _reply(
            ...
            "«Согласование»), кроме отложенных на конкретное время — те "
