...
2. иначе ссылка на новость вложением — VK соберёт карточку из og:image
   источника (промежуточный шаг, не финальный успех — см. ниже).
...
Публикация без картинки, когда картинка была нужна, — не считается успехом.
Если новость должна выйти с фото (как в Telegram), а после публикации в
записи реально нет photo-вложения (VKClient.post перечитывает запись через
_post_has_photo — сам факт того, что wall.post не бросил ошибку, ничего не
гарантирует, см. "Проверенный тупик" выше), публикация в VK отменяется:
уже созданная запись удаляется, наружу летит VKError. VK — второстепенный
канал, поэтому Telegram это не останавливает; администратор просто увидит
явный отказ вместо тихой публикации без картинки.
"""
...
class VKClient:
        ...
        for attempt in range(1, self.retries + 2):
            try:
                ...
                if "error" in data:
                    ...
                    if code == FLOOD_CODE:
                        if attempt > 1:
                            # До этого попытка уже падала по таймауту/сетевой
                            # ошибке (иначе не дошли бы до повтора) — флуд-контроль
                            # тут, скорее всего, означает, что та попытка на самом
                            # деле опубликовалась, а не что пост потерян.
                            raise VKError(f"похоже, пост уже опубликован — VK "
                                          f"отклонил повтор флуд-контролем ({msg})")
    ...
    async def _post_has_photo(self, post_id: int) -> bool:
        """Перечитывает уже опубликованную запись и смотрит, есть ли в ней
        настоящее вложение-фото (attachments[].type == "photo").

        Не доверяем самому факту "wall.post не бросил ошибку" — см.
        "Проверенный тупик" в шапке модуля: VK способен принять
        photo-вложение без единой ошибки API и всё равно не показать его в
        опубликованной записи. Единственный надёжный способ — переспросить
        сам пост. Карточка-ссылка (VK сам собрал превью по og:image) сюда не
        считается: у неё attachments[].type == "link", а не "photo" — ради
        внешнего вида "как в Telegram" (настоящее фото, не карточка-ссылка)
        нам нужен именно photo.
        """
        resp = await self._call("wall.getById", posts=f"{self.owner_id}_{post_id}")
        # 5.131 отдаёт список, свежие версии — {"items": [...]}.
        posts = resp.get("items") if isinstance(resp, dict) else resp
        if not isinstance(posts, list) or not posts:
            return False
        attachments = posts[0].get("attachments") or []
        return any(isinstance(a, dict) and a.get("type") == "photo" for a in attachments)

    async def post(self, text: str, image: str = "", link: str = "",
                   images: list[tuple[bytes, str]] | None = None) -> tuple[int | None, str]:
        """Публикует запись на стену. Возвращает (id записи, "") при успехе.

        Если картинка была запрошена (image/images не пусты), запись обязана
        выйти с настоящим фото-вложением — как в Telegram, не карточкой-
        ссылкой и не голым текстом. После wall.post запись перечитывается
        (_post_has_photo): если фото в ней всё же нет — публикация ОТМЕНЯЕТСЯ
        (запись удаляется, если успела создаться) и наружу летит VKError,
        а не тихий возврат с пустым/битым результатом. Раньше в этом случае
        запись просто оставалась в VK без картинки, и вызывающий код
        (Publisher.send_vk) видел это как "успех с предупреждением" —
        теперь это явный отказ, который останавливает публикацию и доходит
        до администратора (last_vk_error).
        ...
        новости: пробуем его вложением только как промежуточный шаг, если
        не удалось загрузить фото напрямую — финальный результат всё равно
        проверяется на настоящее фото, карточка-ссылка сама по себе успехом
        не считается.
        """
        ...
        if wanted_picture:
            # Картинку просили — проверяем по факту, а не по тому, приняло ли
            # wall.post вложение без ошибки (см. _post_has_photo и
            # "Проверенный тупик" в шапке модуля). Локально уже знаем: если
            # ни одной фотографии не загрузилось (attachments пуст), реальной
            # картинки в записи точно нет — незачем ещё раз спрашивать VK.
            has_photo = bool(attachments)
            if has_photo and post_id:
                try:
                    has_photo = await self._post_has_photo(int(post_id))
                except (VKError, aiohttp.ClientError, asyncio.TimeoutError, ValueError) as exc:
                    # Не смогли подтвердить — не считаем это подтверждением
                    # успеха. Лучше показать администратору отказ и дать
                    # решить вручную, чем оставить не глядя.
                    log.warning("VK: не удалось перепроверить запись wall-%s_%s (%s) — "
                                "считаю, что картинки нет", self.owner_id, post_id, exc)
                    has_photo = False
            if not has_photo:
                if post_id:
                    try:
                        await self._call("wall.delete", owner_id=self.owner_id, post_id=post_id)
                        log.warning("VK: запись wall-%s_%s без картинки — публикация отменена, "
                                    "запись удалена", self.owner_id, post_id)
                    except VKError as exc:
                        log.error("VK: запись wall-%s_%s без картинки, удалить не удалось (%s) "
                                  "— осталась на стене голым текстом!", self.owner_id, post_id, exc)
                raise VKError("в записи после публикации нет картинки — публикация в VK отменена")
