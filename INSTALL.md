...
Если новости нужны ещё и на стене сообщества VK — публикация ручная: бот
после каждого поста в канал сам присылает админам картинку и текст для VK
в личку, ключи в `.env` для этого не нужны (подробно —
[SETUP.md](SETUP.md#публикация-в-vk)).
